...
sudo apt-get install -y nano htop make patch rsync git cmake ninja-build clang$CLANG_VERSION python uuid-dev libicu-dev icu-devtools libbsd-dev libedit-dev libxml2-dev libsqlite3-dev swig libpython-dev libncurses5-dev pkg-config libblocksruntime-dev libcurl4-openssl-dev autoconf libtool systemtap-sdt-dev libcurl4-openssl-dev libz-dev
...
cd swift
git checkout swift-5.2.3-RELEASE
cd -
./swift/utils/update-checkout --clone --tag swift-5.2.3-RELEASE
